...
class DemoCustomDisp { // eslint-disable-line no-unused-vars
	...
	static async render(params) {
		...
		for (const app of user.apps)
			if (app.home != user.home)
				scopes.append($('<a class="text-primary"/>').attr('href', `/ui?scope=${app.home}`).text(app.label)).append($('<span/>').text(' | '));

		const prd = session.getBusinessObject('DemoProduct');
		await prd.getMetaData();
		$('#democustomdisp-products-title').append(/*prd.getPluralLabel()*/'Products');
